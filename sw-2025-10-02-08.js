...
const ASSETS=['./','./index.html','./style.css?v='+VERSION,'./manifest.webmanifest?v='+VERSION,'./version.json?v='+VERSION];
...
self.addEventListener('activate',e=>{
  e.waitUntil(caches.keys().then(keys=>Promise.all(
    keys.map(k=>(k.startsWith('cloverpit-')&&k!==CACHE)?caches.delete(k):null)
  )));
  self.clients.claim();
});
